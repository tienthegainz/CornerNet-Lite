...
def decode(nnet, images, K, ae_threshold=0.5, kernel=3, num_dets=1000):
    detections = nnet.test([images], ae_threshold=ae_threshold,
                           test=True, K=K, kernel=kernel, num_dets=num_dets)[0]
...
def cornernet_inference(db, nnet, image, decode_func=decode):
    ...
    for scale in scales:
        ...
        out_height, out_width = (
            inp_height + 1) // height_scale, (inp_width + 1) // width_scale
        ...
        dets = decode_func(nnet, images, K, ae_threshold=ae_threshold,
                           kernel=nms_kernel, num_dets=num_dets)
        if test_flipped:
    ...
    for j in range(categories):
        ...
        if merge_bbox:
            soft_nms_merge(top_bboxes[j + 1], Nt=nms_threshold,
    ...
    scores = np.hstack([top_bboxes[j][:, -1]
                        for j in range(1, categories + 1)])

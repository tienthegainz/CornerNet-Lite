...
def init_parallel_jobs(system_config, dbs, queue, fn, data_aug):
    tasks = [Process(target=prefetch_data, args=(
        system_config, db, queue, fn, data_aug)) for db in dbs]
    for task in tasks:
        task.daemon = True
        task.start()
    return tasks
...
def train(training_dbs, validation_db, system_config, model, args):
    ...
    training_tasks = init_parallel_jobs(
        system_config, training_dbs, training_queue, data_sampling_func, True)
    if val_iter:
        validation_tasks = init_parallel_jobs(
            system_config, [validation_db], validation_queue, data_sampling_func, False)
    ...
    if start_iter:
        ...
        print("Process {}: training starts from iteration {} with learning_rate {}".format(
            rank, start_iter + 1, learning_rate))
    else:
        nnet.set_lr(learning_rate)
...
if __name__ == "__main__":
    ...
    if distributed and world_size < 0:
        raise ValueError(
            "world size must be greater than 0 in distributed training")
